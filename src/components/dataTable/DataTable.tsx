...
const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || "https://backend-server-mkzqlowyp-saifd227s-projects.vercel.app";
...
const DataTable = () => {
  ...
  const handleUpdate = async () => {
    if (!editingProduct) return;
    try {
      await fetch(`${API_BASE_URL}/api/products/${editingProduct._id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(editingProduct),
      });
